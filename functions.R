...
find_csv('/Users/doblakov/R/ROblakov')
...
setMethod(
  ...
  function(object) {
    ...
    cat("Year of Publication:", object@year, "\n")
    cat("Title:", object@title, "\n")
    cat("Cover Color:", object@cover_color, "\n")
  }
)
...
setClass(
  ...
  contains = "book",
  slots = c(
    history_period = "character"
  )
)

# Метод печати для класса documentary_book, который вызывает метод печати класса book и добавляет дополнительный параметр.
setMethod(
  "show",
  "documentary_book",
  function(object) {
    callNextMethod()
    cat("History Period:", object@history_period, "\n")
  }
)

# Пример создания объекта класса book.
book1 <- new("book", author = "George Orwell", year = 1949, title = "1984", cover_color = "Blue")
show(book1)

# Пример создания объекта класса documentary_book.
doc_book1 <- new("documentary_book", author = "Yuval Noah Harari", year = 2011, title = "Sapiens: A Brief History of Humankind", cover_color = "White", history_period = "Ancient to Modern Times")
show(doc_book1)
